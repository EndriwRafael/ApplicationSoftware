...
class Ui_WindButton_LonLatProfile(object):
    ...
    def plot_var_profile_withdepth(self):
        ...
        dataset_to_plot = self.dataset.sel({self.lat_name: float(self.comboBox.currentText()), self.time_name: self.time_selected})

        var_value = dataset_to_plot[self.sali_name].values
        depth = dataset_to_plot[self.depth_name].values
        x_axis = dataset_to_plot[self.lon_name].values
        ...
        x_axis_filtered = [value - 360 for value in x_axis_filtered]
    ...
    def save_fig(self):
        ...
        t_formated = datetime.strptime(time_to_format, '%Y-%m-%dT%H:%M:%S').strftime('%Y-%m-%d-%Hh')
        file_path = os.path.join(path_to_save, 'DEPTHvsLON_SALINITY')

        plt.savefig(f'{file_path}_{t_formated}_LATcoord{self.comboBox.currentText()}.png',
                    transparent=True)
        plt.close()
    ...
    def retranslateUi(self, WindButton_LonLatProfile):
        ...
        self.TimeFilterLabel_2.setText(
            QCoreApplication.translate("WindButton_LonLatProfile", u"Latitude coordinate", None))
